...
<div class="relative w-full mb-8">
                ...
                <div v-if="advancedInputs.length" class="mt-4">
                    ...
                    <div v-show="prefs.showAdvanced" class="mt-3 p-4 bg-gray-50 dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 shadow-sm">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div v-for="input in advancedInputs" class="flex flex-col space-y-1" :class="input.type === 'String' && input.classType == 'CLIPTextEncode' ? 'col-span-2 row-span-2' : ''">
                                ...
                                <textarea v-if="input.type === 'String' && input.classType == 'CLIPTextEncode'" spellcheck="false"
    ...
    setup(props, { emit, expose }) {
        ...
        function hasInput(...inputs) {
            if (!props.selectedWorkflowInfo?.inputs?.length) {
                return false
            }
            for (const input of inputs) {
                if (!props.selectedWorkflowInfo.inputs.find(x => x.name === input)) {
                    return false
                }
            }
            return true
        ...
        function resetPositivePrompt() {
            ...
            const denoise = props.selectedWorkflowInfo.inputs.find(x => x.name === 'denoise')?.default
            if (denoise !== undefined) {
                props.workflowArgs.denoise = denoise
            }
            const cfg = props.selectedWorkflowInfo.inputs.find(x => x.name === 'cfg')?.default
            if (cfg !== undefined) {
                props.workflowArgs.cfg = cfg
            }
            const steps = props.selectedWorkflowInfo.inputs.find(x => x.name === 'steps')?.default
            if (steps !== undefined) {
                props.workflowArgs.steps = steps
            }
            const sampler_name = props.selectedWorkflowInfo.inputs.find(x => x.name === 'sampler_name')?.default
            if (sampler_name !== undefined) {
                props.workflowArgs.sampler_name = sampler_name
            }
            const scheduler = props.selectedWorkflowInfo.inputs.find(x => x.name === 'scheduler')?.default
            if (scheduler !== undefined) {
                props.workflowArgs.scheduler = scheduler
            }
            // const seed = props.selectedWorkflowInfo.inputs.find(x => x.name === 'seed')?.default
            // if (seed !== undefined) {
            //     props.workflowArgs.seed = seed
            // }
            // console.log(JSON.stringify(props.selectedWorkflowInfo))
        }
