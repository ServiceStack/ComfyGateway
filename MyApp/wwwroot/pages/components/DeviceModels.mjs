...
import { SecondaryButton, SelectInput, useClient, useUtils } from "@servicestack/vue"
...
export default {
                  ...
                  <div v-for="model in category.models" :key="model"
                    ...
                    <div class="flex items-center">
                      ...
                      <span class="text-sm font-mono break-all" :title="category.key + '/' + model"
                            :class="installer.isModelUninstalling(category.key + '/' + model) ? 'line-through text-red-400 dark:text-red-500' : 'text-gray-700 dark:text-gray-300 group-hover:text-black dark:group-hover:text-white'">
                              {{ getFileName(model) }}
                        </span>
                    </div>
    ...
    setup(props) {
        ...
        function createModelCategories(device) {
            ...
            return Object.keys(device.models ?? {}).map((category, i) => ({
                key: category,
