...
let o = {
    ...
    async openAppDb() {
        ...
        this._dbApp = await openDB(AppDatabase, 1, {
            upgrade(db) {
                ...
                createStore(db, Tables.WorkflowInfo, { keyPath: 'id' }, {
                    indexes: ['createdBy']
                })
    ...
    async reload() {
        ...
        const hasGenerations = db ? (await db.count(Tables.WorkflowGeneration) ?? 0) > 0 : false
    ...
    async getWorkflowInfo(versionId) {
        let ret = await this.get(Tables.WorkflowInfo, Number(versionId))
        if (ret) return ret
        const api = await this._client.api(new GetWorkflowInfo({ versionId }))
        ret = api.response?.result
        if (ret) {
            await this.put(Tables.WorkflowInfo, ret)
        }
        return ret
    },
    ...
    selectWorkflow(args) {
        localStorage.setItem(this.workflowKey, JSON.stringify(args))
    },
