    ...
    AppData appData, AppConfig appConfig, AgentEventsManager agentEvents, IDbConnectionFactory dbFactory) : Service
{
    ...
    public object Any(GenerateCaptionArtifact request)
    {
        ...
        var model = request.Model ?? appData.Config.VisualLanguageModel;
        var nextArtifacts = db.Select(Db.From<Artifact>()
            .Where(x => x.Type == AssetType.Image 
                && x.Caption == null || x.Description == null 
                && (request.ArtifactIds == null || request.ArtifactIds.Contains(x.Id)))
            .OrderBy(x => x.Id)
            .Take(request.Take ?? 1));

        var existingCaptionArtifactIds = dbTasks.ColumnDistinct<long>(dbTasks.From<OllamaGenerateTask>()
            .Where(x => x.Task == AiTasks.CaptionImage)
            .Select(x => x.TaskId));
        var existingDescribeArtifactIds = dbTasks.ColumnDistinct<long>(dbTasks.From<OllamaGenerateTask>()
            .Where(x => x.Task == AiTasks.DescribeImage)
            .Select(x => x.TaskId));
        
        var ret = new StringsResponse();
        foreach (var artifact in nextArtifacts)
        {
            if (artifact.Caption == null && !existingCaptionArtifactIds.Contains(artifact.Id))
            {
                var task = agentEvents.AddCaptionArtifactTask(dbTasks, artifact, userId, model);
                ret.Results.Add($"{task.Task} {task.TaskId}");
            }

            if (artifact.Description == null && !existingDescribeArtifactIds.Contains(artifact.Id))
            {
                var task = agentEvents.AddDescribeArtifactTask(dbTasks, artifact, userId, model);
                ret.Results.Add($"{task.Task} {task.TaskId}");
            }
        }
        return ret;
