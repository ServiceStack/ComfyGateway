...
public class ThreadServices(IAutoQueryDb autoQuery, IBackgroundJobs jobs, AppConfig appConfig) : Service
{
    ...
    public Thread Post(CreateThread request)
    {
        ...
        thread.Id = (int) Db.Insert(thread, selectIdentity: true);
        if (request.Url.Contains("{id}") || request.Url.Contains("{Id}"))
        {
            ...
            Db.UpdateOnly(() => new Thread { Url = url }, where: x => x.Id == thread.Id);
        }
        return thread;
    }
    ...
    public object Post(CreateComment request)
    {
        ...
        if (thread != null)
        {
            var replyUserId = request.ReplyId != null
                ? Db.SingleById<Comment>(request.ReplyId.Value)?.UserId
                : null;
